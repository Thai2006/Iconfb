...
const en = {
  ...
  "share.error.empty": "Note cannot be empty unless you select a music track",
  "share.error.missing_song": "Selected track is missing song ID. Please reselect music.",
...
const vi = {
  ...
  "share.error.missing_song": "Bài nhạc đang thiếu song ID. Vui lòng chọn lại.",
  ...
  "status.share.custom.with_names_more": "Chia sẻ với: {names} +{remaining}",
  "status.expiry.unknown": "Không rõ thời gian hết hạn",
  "status.expiry.with_days": "Hết hạn: {date} (còn {days} ngày {hours} giờ)",
...
const formatDuration = (durationMs) => {
  if (durationMs === void 0 || durationMs === null) return "--:--";
...
const App = () => {
  ...
  const [tokenStatus, setTokenStatus] = reactExports.useState("loading");
  const [noteText, setNoteText] = reactExports.useState("");
  const [duration, setDuration] = reactExports.useState(86400);
  const [customDurationMinutes, setCustomDurationMinutes] = reactExports.useState("");
  const [audienceSetting, setAudienceSetting] = reactExports.useState("FRIENDS");
  const [friendQuery, setFriendQuery] = reactExports.useState("");
  const [friendItems, setFriendItems] = reactExports.useState([]);
  const [friendLoading, setFriendLoading] = reactExports.useState(false);
  const [friendNextCursor, setFriendNextCursor] = reactExports.useState(null);
  const [friendHasNextPage, setFriendHasNextPage] = reactExports.useState(false);
  const [selectedFriendIds, setSelectedFriendIds] = reactExports.useState([]);
  const [selectedFriends, setSelectedFriends] = reactExports.useState([]);
  const [musicQuery, setMusicQuery] = reactExports.useState("");
  const [musicItems, setMusicItems] = reactExports.useState([]);
  const [musicLoading, setMusicLoading] = reactExports.useState(false);
  const [visibleMusicCount, setVisibleMusicCount] = reactExports.useState(MUSIC_PAGE_SIZE);
  const [selectedMusic, setSelectedMusic] = reactExports.useState(null);
  const [playingMusicId, setPlayingMusicId] = reactExports.useState(null);
  const audioRef = reactExports.useRef(null);
  const [musicTrimStartMs, setMusicTrimStartMs] = reactExports.useState(0);
  const [musicTrimWindowMs] = reactExports.useState(3e4);
  const [previewPlaying, setPreviewPlaying] = reactExports.useState(false);
  const [previewProgressMs, setPreviewProgressMs] = reactExports.useState(0);
  ...
  const [encodedLength, setEncodedLength] = reactExports.useState(0);
  const [currentNoteStatus, setCurrentNoteStatus] = reactExports.useState(null);
  const [currentStatusLoading, setCurrentStatusLoading] = reactExports.useState(false);
  const [isDeleting, setIsDeleting] = reactExports.useState(false);
  const [activeModal, setActiveModal] = reactExports.useState(null);
  const [showFriendsModal, setShowFriendsModal] = reactExports.useState(false);
  const [language, setLanguage] = reactExports.useState(resolveInitialLanguage());
  const [showLanguageMenu, setShowLanguageMenu] = reactExports.useState(false);
  const musicListRef = reactExports.useRef(null);
  const friendsListRef = reactExports.useRef(null);
  const bubbleRef = reactExports.useRef(null);
  const t = reactExports.useMemo(() => createTranslator(language), [language]);
  reactExports.useEffect(() => {
    chrome.storage.local.get([POPUP_LANGUAGE_KEY], (res) => {
      const saved = res?.[POPUP_LANGUAGE_KEY];
  ...
  const visibleMusicItems = reactExports.useMemo(
  ...
  const handleSearchMusic = reactExports.useCallback((query) => {
    ...
    chrome.runtime.sendMessage({
      ...
      if (response?.success) {
        ...
        console.log("Music items received:", items.map((i) => ({ id: i.id, title: i.title, hasUrl: !!i.progressiveDownloadUrl, url: i.progressiveDownloadUrl?.slice(0, 50) })));
        setMusicItems(items);
  ...
  const handlePlayMusic = reactExports.useCallback((item) => {
  ...
  const handlePreviewPlayToggle = reactExports.useCallback(() => {
    ...
    audioRef.current = audio;
    audio.currentTime = Math.max(0, startMs / 1e3);
    setPreviewProgressMs(0);
    setPreviewPlaying(true);
    audio.ontimeupdate = () => {
      const currentStartMs = musicTrimStartMsRef.current;
      const currentMs = audio.currentTime * 1e3;
  ...
  reactExports.useEffect(() => {
    ...
    if (audio && selectedMusic) {
      ...
      audio.currentTime = Math.max(0, newStartMs / 1e3);
      setPreviewProgressMs(0);
    }
  }, [musicTrimStartMs, previewPlaying, selectedMusic, musicTrimWindowMs]);
  const handleSearchFriends = reactExports.useCallback((query, cursor = null) => {
    if (!tokens || tokenStatus !== "ready") return;
  ...
  const toggleFriendSelection = reactExports.useCallback((friend) => {
  ...
  const removeSelectedFriend = reactExports.useCallback((friendId) => {
    setSelectedFriendIds((prev) => prev.filter((id) => id !== friendId));
    setSelectedFriends((prev) => prev.filter((f) => f.id !== friendId));
  }, []);
  const applyCustomDuration = reactExports.useCallback((minutesText) => {
  ...
  const musicTrimDragRef = reactExports.useRef(null);
  const musicWaveContainerRef = reactExports.useRef(null);
  const musicTrimStartMsRef = reactExports.useRef(0);
  const isDraggingTrimRef = reactExports.useRef(false);
  const previewPlayingRef = reactExports.useRef(false);
  const selectedMusicRef = reactExports.useRef(null);
  reactExports.useEffect(() => {
  ...
  const clampMusicTrimStart = reactExports.useCallback((valueMs, durationMs) => {
  ...
  const startMusicTrimDrag = reactExports.useCallback((clientX) => {
  ...
  reactExports.useEffect(() => {
    ...
    const handleUp = () => {
      ...
      if (currentPreviewPlaying && currentAudio && currentSelectedMusic) {
        ...
        currentAudio.currentTime = Math.max(0, newStartMs / 1e3);
        setPreviewProgressMs(0);
      }
      setTimeout(() => {
  ...
  const handleMusicListScroll = reactExports.useCallback(() => {
  ...
  const handleFriendsListScroll = reactExports.useCallback(() => {
  ...
  const handleSubmit = reactExports.useCallback(async () => {
    ...
    chrome.runtime.sendMessage({
      ...
      selectedFriendIds,
      selectedMusic,
      musicTrimStartMs: Math.floor(musicTrimStartMs / 1e3) * 1e3
  ...
  const shareLabel = reactExports.useMemo(() => {
    ...
    if (visibility === "CUSTOM") {
      ...
      return remaining > 0 ? t("status.share.custom.with_names_more", { names: first, remaining }) : t("status.share.custom.with_names", { names: first });
    }
    return t("status.share.default");
  }, [currentNoteStatus, isPreviewPlaceholder, t]);
  const expiryLabelShort = reactExports.useMemo(() => {
    if (isPreviewPlaceholder) return "";
    const ts = currentNoteStatus?.expirationTime;
    if (!ts) return "";
    const target = new Date(ts * 1e3);
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "container", children: [
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "note-preview-stage", children: [
      canDeleteNote && /* @__PURE__ */ jsxRuntimeExports.jsx(
        "button",
        {
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "note-bubble-preview", ref: bubbleRef, children: [
        currentNoteStatus?.musicTitle && !currentStatusLoading && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bubble-music-title-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(Music, { size: 13 }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: currentNoteStatus.musicTitle })
        ] }),
        currentNoteStatus?.musicArtist && !currentStatusLoading && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bubble-music-artist-row", children: currentNoteStatus.musicArtist }),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            className: `bubble-note-content ${isPreviewPlaceholder ? "is-placeholder" : ""} ${currentStatusLoading ? "is-shimmer" : ""}`,
            children: currentStatusLoading ? "" : previewText
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "bubble-pointer-dots", "aria-hidden": "true", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "pointer-dot dot-large" }),
          /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "pointer-dot dot-small" })
        ] })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "note-avatar-preview", children: currentStatusLoading ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "note-avatar-img avatar-shimmer" }) : currentNoteStatus?.avatarUri ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: currentNoteStatus.avatarUri, alt: "Avatar", className: "note-avatar-img" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "note-avatar-fallback" }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "note-preview-meta", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bubble-meta-line", children: currentStatusLoading ? t("preview.loading") : shareLabel }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "bubble-meta-line secondary", children: currentStatusLoading ? "" : expiryLabelShort })
      ] })
    ] }),
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "section", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-header", children: /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "section-title", children: t("composer.title") }) }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "section-content", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "note-composer", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "composer-scroll", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "textarea",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "char-counter", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "char-bar-container", children: /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs("span", { className: "char-text", children: [
            encodedLength,
            " / ",
            MAX_DESCRIPTION_LENGTH
          ] })
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "char-counter-footer", children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "action-buttons-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "action-left", "data-lang-menu": true, children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                className: "icon-btn",
            ...
            /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lang-menu-wrapper", "data-lang-menu": true, children: [
              /* @__PURE__ */ jsxRuntimeExports.jsxs(
                "button",
                {
                  ...
                  children: [
                    ...
                    /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "icon-badge", children: language.toUpperCase() })
                  ]
                }
              ),
              showLanguageMenu && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "lang-menu", role: "menu", children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              children: [
                ...
                audienceSetting !== "DEFAULT" && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "icon-badge", children: audienceSetting === "CUSTOM" ? `${selectedFriendIds.length}` : audienceSetting.charAt(0) })
              ]
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              children: [
                ...
                duration !== 86400 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "icon-badge", children: formatDurationFromSeconds(duration) })
              ]
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "button",
            {
              ...
              children: [
                ...
                selectedMusic && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "icon-badge", children: "♪" })
              ]
            }
          ),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: isSubmitting ? /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
                /* @__PURE__ */ jsxRuntimeExports.jsx(Loader2, { size: 12, className: "spinner" }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t("share.submitting") })
              ] }) : /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: t("share.button") })
            }
          )
        ] }) })
      ] }) }) })
    ] }),
    activeModal === "audience" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-overlay", onClick: () => setActiveModal(null), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-content", onClick: (e) => e.stopPropagation(), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-header", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "modal-title", children: t("audience.title") }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "modal-close", onClick: () => setActiveModal(null), children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 16 }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-body", children: /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "audience-options", children: AUDIENCE_OPTIONS.map((opt) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
        "button",
        {
          className: `audience-option ${audienceSetting === opt.value ? "active" : ""}`,
          onClick: () => {
            setAudienceSetting(opt.value);
            if (opt.value === "CUSTOM") {
          ...
          children: [
            t(opt.key),
            opt.value === "CUSTOM" && selectedFriendIds.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "option-badge", children: selectedFriendIds.length })
          ]
        },
        opt.value
      )) }) })
    ] }) }),
    activeModal === "duration" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-overlay", onClick: () => setActiveModal(null), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-content modal-small", onClick: (e) => e.stopPropagation(), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-header", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "modal-title", children: t("duration.title") }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "modal-close", onClick: () => setActiveModal(null), children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 16 }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-body", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "duration-selector", children: DURATION_OPTIONS.map((opt) => /* @__PURE__ */ jsxRuntimeExports.jsx(
          "button",
          {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "duration-custom-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: t("duration.apply")
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "duration-current", children: t("duration.current", { duration: formatDurationFromSeconds(duration) }) })
      ] })
    ] }) }),
    activeModal === "music" && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-overlay", onClick: () => {
      if (audioRef.current) {
        audioRef.current.pause();
        audioRef.current = null;
      }
      ...
      setPreviewProgressMs(0);
      setActiveModal(null);
    }, children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-content", onClick: (e) => e.stopPropagation(), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-header", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "modal-title", children: t("music.title") }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-header-actions", children: [
          selectedMusic && /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "modal-close", onClick: () => {
            if (audioRef.current) {
              audioRef.current.pause();
              audioRef.current = null;
            }
      ...
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-body", children: [
        selectedMusic && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "music-selected", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "music-selected-text", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("strong", { children: selectedMusic.title }),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: selectedMusic.artist || "Unknown artist" })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              children: t("music.clear")
            }
          )
        ] }),
        selectedMusic && /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "music-trim", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "music-trim-time", children: [
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatDuration(musicTrimStartMs + previewProgressMs) }),
            /* @__PURE__ */ jsxRuntimeExports.jsx(
              "button",
              {
                ...
                children: previewPlaying ? /* @__PURE__ */ jsxRuntimeExports.jsx(Pause, { size: 14 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 14 })
              }
            ),
            /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: formatDuration((selectedMusic.durationMs || 0) - musicTrimStartMs - previewProgressMs) })
          ] }),
          /* @__PURE__ */ jsxRuntimeExports.jsxs(
            "div",
            {
        ...
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "music-search-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              disabled: musicLoading,
              children: musicLoading ? "..." : t("music.search")
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "music-list", ref: musicListRef, onScroll: handleMusicListScroll, children: [
          visibleMusicItems.map((item) => /* @__PURE__ */ jsxRuntimeExports.jsx(
            "div",
            {
              className: `music-item ${selectedMusic?.id === item.id ? "active" : ""}`,
              onClick: () => setSelectedMusic(item),
              children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "music-item-top", children: [
                item.imageUri ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { src: item.imageUri, alt: item.title, className: "music-cover", loading: "lazy" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "music-cover music-cover-placeholder", children: "♪" }),
                /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "music-item-text", children: [
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "music-item-title", children: item.title }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "music-item-artist", children: item.artist || "Unknown artist" })
                ] }),
                /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "music-item-duration", children: formatDuration(item.durationMs) }),
                /* @__PURE__ */ jsxRuntimeExports.jsx(
                  "button",
                  {
                    ...
                    type: "button",
                    children: playingMusicId === item.id ? /* @__PURE__ */ jsxRuntimeExports.jsx(Pause, { size: 14 }) : /* @__PURE__ */ jsxRuntimeExports.jsx(Play, { size: 14 })
                  }
                )
              ] })
            },
          ...
          !musicLoading && musicItems.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "music-empty", children: t("music.empty") }),
          musicItems.length > visibleMusicCount && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "music-loading-more", children: t("music.load_more") })
        ] })
      ] })
    ] }) }),
    showFriendsModal && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "modal-overlay", onClick: () => setShowFriendsModal(false), children: /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-content", onClick: (e) => e.stopPropagation(), children: [
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-header", children: [
        /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "modal-title", children: t("friends.title", { count: selectedFriendIds.length }) }),
        /* @__PURE__ */ jsxRuntimeExports.jsx("button", { className: "modal-close", onClick: () => setShowFriendsModal(false), children: /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 16 }) })
      ] }),
      /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "modal-body", children: [
        selectedFriends.length > 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "selected-friends-chips", children: selectedFriends.map((friend) => /* @__PURE__ */ jsxRuntimeExports.jsxs(
          "button",
          {
            className: "friend-chip",
            onClick: () => removeSelectedFriend(friend.id),
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { children: friend.name }),
              /* @__PURE__ */ jsxRuntimeExports.jsx(X, { size: 12 })
            ]
          },
          friend.id
        )) }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "friends-search-row", children: [
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "input",
            {
          ...
          /* @__PURE__ */ jsxRuntimeExports.jsx(
            "button",
            {
              ...
              disabled: friendLoading,
              children: friendLoading ? "..." : t("friends.search")
            }
          )
        ] }),
        /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "friends-list", ref: friendsListRef, onScroll: handleFriendsListScroll, children: [
          friendItems.map((friend) => {
            ...
            return /* @__PURE__ */ jsxRuntimeExports.jsxs(
              "button",
              {
                ...
                children: [
                  friendLoading ? /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "friend-avatar avatar-shimmer" }) : friend.imageUri ? /* @__PURE__ */ jsxRuntimeExports.jsx("img", { className: "friend-avatar", src: friend.imageUri, alt: friend.name, loading: "lazy" }) : /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "friend-avatar friend-avatar-placeholder", children: "👤" }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "friend-name", children: friend.name }),
                  /* @__PURE__ */ jsxRuntimeExports.jsx("span", { className: "friend-check", children: active ? "✓" : "" })
                ]
              },
              friend.id
            );
          }),
          friendLoading && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "music-loading-more", children: t("friends.loading") }),
          !friendLoading && friendItems.length === 0 && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "music-empty", children: t("friends.empty") })
        ] }),
        friendHasNextPage && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: "friends-pagination-hint", children: t("music.load_more") })
      ] })
    ] }) }),
    showToast && result && /* @__PURE__ */ jsxRuntimeExports.jsx("div", { className: `toast ${result.type}`, children: result.message })
  ] });
};
